...
Two more real, still-open hardware issues, both surfaced by validating
the timebase change above against actual angle *values* on real hardware
-- not just protocol shape -- via `python/tests/test_hardware.py`'s
`ClosedLoopCamTests`, which checks captured cam angle against its
known-true 120/300deg (cam is generated by this same firmware, so its
true angle is known independent of anything captured) rather than just
presence/shape the way the rest of that file's tests do:

(a) On some fraction of fresh sessions (`g1` immediately followed by
`c1`), EVERY capture channel -- not just one -- comes back completely
undetected, starting within the first few cycles and persisting for the
rest of that session. `have_refs` stays `true` throughout, so this is
squarely a capture-pipeline problem (raw edge sampling never happening),
not a timebase/reference one. This looks like the same underlying issue
as an OLDER, narrower finding from before the timebase change (a fresh
`g1` + loopback-wired capture sometimes finding the crank input pin stuck,
attributed then to `event_gen.pio`'s generation-side output-enable
timing) -- but the new evidence (every channel affected simultaneously,
not just crank) points at the capture side, or at something shared
between generation and capture, rather than a crank-output-specific drive
issue. A firmware fix was tried (re-init the capture PIO SM on every
`engine_start_capture()`, not just once at boot -- see (b) below) and
measured on hardware to make this dropout MORE frequent across repeated
full-suite runs, not less -- reverted (see git history). Root cause
remains unresolved; needs a scope on the raw GPIO6-11 signals during a
failing session, not just protocol-level testing.

(b) Separately, and much narrower: even on an otherwise-healthy session,
the very first reported cycle can show one bogus edge (e.g. a ~180deg
pulse's fall angle reported near 0deg instead of its true value) that
never recurs from cycle 2 onward. Root-caused to `engine_stop_capture()`
only pausing the capture PIO SM (`pio_sm_set_enabled(pio_cap, sm_capture,
false)`), not resetting it -- it resumes on the next
`engine_start_capture()` wherever it was frozen mid `in pins,6` / `push`,
occasionally misaligning sample 0 of the new session's first buffer
against the real GPIO state by a fraction of a sample period. The tried
fix (`capture_program_init()` called again at the start of every
`engine_start_capture()`, not just once at boot in `engine_init()`, so
the SM always resumes from `.wrap_target`) measurably eliminated this
specific glitch (0/60 across two batches of hardware trials, vs. an
originally-estimated ~50% rate) -- but is the same fix that worsened (a),
so it was reverted alongside it rather than landing a narrow win that
makes the bigger problem worse. Worth retrying once (a) is understood,
not before.

Given both are still open, `ClosedLoopCamTests` treats a whole-session
(or whole-RPM-step) dropout as issue (a) and skips with a clear reason
instead of failing, and discards the first cycle of a session (issue (b))
and the first two cycles after any live RPM change (the ordinary
consequence of capture's independent DMA chain, below, needing a cycle or
two to resync after a rate change) as settle/warm-up cycles before
asserting per-cycle angle correctness on what's left. A genuine angle
miss outside those windows, or a channel undetected in only a minority of
cycles, still fails the test normally -- see its docstring.
...
edges, there's no more discrete window for a *correctly-captured* edge to
land in the wrong one of. `convert_time_to_angle` only returns the
"--"/no-angle sentinel if a sample's timestamp predates every known
`CycleBoundary` -- still expected during the settle windows (a)/(b) above
describe, but not as a routine per-cycle artifact otherwise.
`python/crankcam/board.py`'s `ChannelReport.detected` still distinguishes
"no edge at all" (a channel that's genuinely unwired, or hit by issue (a)
above) from "an edge was found but its angle came back as the sentinel"
(issue (b), or mid-settle after an RPM change).
